...
public class MeshChunk {
	...
	public MeshChunk(PreparedChunk preparedChunk, int opaqueElements, int opaqueElementOffset,
			int bytesPerOpaqueElement, int transparentElements, int transparentElementOffset,
			int bytesPerTransparentElement, boolean useTransparency) {
		...
		if (useTransparency)
			this.transparentBuffers = Optional.of(
					new MeshChunk.BufferSet(transparentElements, transparentElementOffset, bytesPerTransparentElement));
		else
	...
	public static class BufferSet {
		...
		public void ensureCapacity(int requiredElements) {
			if (requiredElements <= this.maxElements)
				return;

			int grownElements = Math.max(requiredElements,
					Math.max(4, this.maxElements + Math.max(4, this.maxElements / 2)));
			int newBufferSize = grownElements * this.bytesPerElement;
			ByteBuffer newBuffer = MemoryUtil.memAlloc(newBufferSize);

			GL15.glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, this.bufferId);
			GL15.glBufferData(GL43.GL_SHADER_STORAGE_BUFFER, newBuffer, GL15.GL_DYNAMIC_DRAW);
			GL15.glBindBuffer(GL43.GL_SHADER_STORAGE_BUFFER, 0);

			if (this.buffer != null)
				MemoryUtil.memFree(this.buffer);
			this.buffer = newBuffer;
			this.maxElements = grownElements;
