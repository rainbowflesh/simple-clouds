...
public class RendererInitializeResult {
	...
	private RendererInitializeResult(RendererInitializeResult.State state,
			List<RendererInitializeResult.Error> errors) {
		this.state = state;
		this.errors = errors;
	}
	...
	public List<CrashReport> createCrashReports() {
		...
		for (RendererInitializeResult.Error error : this.errors) {
			Throwable cause = error.error();
			if (cause == null)
				cause = new IllegalStateException(error.title());
			CrashReport report = CrashReport.forThrowable(cause,
					"Simple Clouds mesh generator initialization; " + error.title());
			...
			if (this.errors.size() > 1)
				category.setDetail("Notice",
						"Multiple crash reports have been generated during mesh generator initialization");
			reports.add(report);
		}
	...
	public static class Builder {
		...
		public Builder errorOpenGL() {
			return this.addError(null, "Outdated OpenGL Version",
					Component.translatable("gui.simpleclouds.error.opengl"));
		}
		...
		public Builder coreShadersNotInitialized(@Nullable Throwable error) {
			return this.addError(error, "Core Shader Initialization Error",
					Component.translatable("gui.simpleclouds.error.coreShadersInitialization"));
		}
