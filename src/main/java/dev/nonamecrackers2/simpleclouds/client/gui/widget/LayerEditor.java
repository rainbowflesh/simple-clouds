...
public class LayerEditor extends ContainerObjectSelectionList<LayerEditor.Entry> {
	...
	public LayerEditor(ModifiableNoiseSettings settings, Minecraft mc, int width, int height, int headerHeight,
			Runnable onChanged) {
	...
	public void buildEntries() {
		...
		for (AbstractNoiseSettings.Param parameter : AbstractNoiseSettings.Param.values())
			if (parameter != AbstractNoiseSettings.Param.HEIGHT_OFFSET)
	...
	public class Entry extends ContainerObjectSelectionList.Entry<LayerEditor.Entry> {
		...
		public Entry(AbstractNoiseSettings.Param parameter) {
			...
			var fullName = Component.translatable(
					"gui.simpleclouds.noise_settings.param." + parameter.toString().toLowerCase() + ".name");
			...
			this.tooltip = Lists
					.newArrayList(fullName,
							Component
									.translatable("gui.simpleclouds.noise_settings.param.range",
											parameter.getMinInclusive(), parameter.getMaxInclusive())
		...
		@Override
		public void render(GuiGraphics stack, int index, int top, int left, int width, int height, int mouseX,
				int mouseY, boolean selected, float partialTicks) {
			this.prevLeft = left;
			this.prevTop = top;
		...
		public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
			...
			if (button != 0)
				return false;
			if (RenderUtil.isMouseInBounds((int) mouseX, (int) mouseY, this.prevLeft, this.prevTop, 20 + TEXT_WIDTH,
					ROW_HEIGHT)) {
