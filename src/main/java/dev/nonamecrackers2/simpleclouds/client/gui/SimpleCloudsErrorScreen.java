...
public class SimpleCloudsErrorScreen extends SimpleCloudsInfoScreen {
	...
	public SimpleCloudsErrorScreen(RendererInitializeResult result, Runnable onClose) {
		super(Component.translatable("gui.simpleclouds.error_screen.title")
	...
	protected void generateText(List<FormattedCharSequence> text, int maxWidth) {
		...
		if (!this.result.getErrors().isEmpty()) {
			...
			if (this.result.getErrors().size() > 1) {
				text.add(FormattedCharSequence.EMPTY);
				text.addAll(
						this.font.split(Component.translatable("gui.simpleclouds.error_screen.multiple"), maxWidth));
			}
		} else {
	...
	protected void generateButtons(RowHelper row) {
		...
		Button button = row.addChild(
				Button.builder(Component.translatable("gui.simpleclouds.error_screen.button.crash_report"), b -> {
		...
		button.active = this.result.getSavedCrashReportPaths() != null
				&& !this.result.getSavedCrashReportPaths().isEmpty();
	}
			...
			this.minecraft.reloadResourcePacks().thenRunAsync(() -> {
				...
				if (result != null && result.getState() == RendererInitializeResult.State.ERROR)
					this.minecraft.setScreen(
							new SimpleCloudsErrorScreen(renderer.getInitialInitializationResult(), this.onClose));
				else
					this.onClose();
			}, this.minecraft);
			return true;
