...
public class ClientCloudCommandHelper {
	private static final SimpleCommandExceptionType ERROR_NOT_CLIENT_ONLY = new SimpleCommandExceptionType(
			Component.translatable("commands.simpleclouds.notClientSideOnly"));
	private static final SimpleCommandExceptionType CONFIG_REFERAL = new SimpleCommandExceptionType(
			Component.translatable("commands.simpleclouds.client.configReferal"));
	...
	public static final CloudCommandSource<ClientLevel, ClientCloudManager> SOURCE = new CloudCommandSource<>() {
		...
		@Override
		public ClientCloudManager getCloudManager(CommandContext<CommandSourceStack> context)
				throws CommandSyntaxException {
			ClientCloudManager manager = (ClientCloudManager) CloudManager
					.get(Objects.requireNonNull(Minecraft.getInstance().level, "Client level is not available"));
	...
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		CloudCommands.register(dispatcher, "clientClouds", src -> true, SOURCE,
				ClientSideCloudTypeManager.getInstance());
	}
}
