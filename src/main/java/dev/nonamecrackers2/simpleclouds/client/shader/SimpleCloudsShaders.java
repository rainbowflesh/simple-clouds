		...
		try {
			...
			event.registerShader(
					new SingleSSBOShaderInstance(event.getResourceProvider(),
							SimpleCloudsMod.id("storm_fog_shadow_map"), DefaultVertexFormat.POSITION, "SideInfoBuffer"),
					s -> {
						stormFogShadowMap = (SingleSSBOShaderInstance) s;
					});
			event.registerShader(new SingleSSBOShaderInstance(event.getResourceProvider(),
					SimpleCloudsMod.id("clouds_shadow_map"), DefaultVertexFormat.POSITION, "SideInfoBuffer"), s -> {
						cloudsShadowMap = (SingleSSBOShaderInstance) s;
					});
			event.registerShader(new ShaderInstance(event.getResourceProvider(), SimpleCloudsMod.id("cloud_region_tex"),
					DefaultVertexFormat.POSITION_TEX), s -> {
						cloudRegionTex = s;
					});
