...
public class SimpleCloudsRenderer implements ResourceManagerReloadListener {
	...
	private final AtmosphericCloudsRenderHandler atmosphericClouds;
	...
	private boolean meshFaceCullingActive;
	private boolean needsReload;
	private int stormFogResolutionDivisor;
	...
	private SimpleCloudsRenderer(CloudsRendererSettings settings, Minecraft mc) {
		...
		this.atmosphericClouds = new AtmosphericCloudsRenderHandler(mc);
	}
	...
	private void prepareMeshGenerator(float partialTicks, double camX, double camZ) {
		...
		if (this.mc.level != null) {
			...
			boolean insideCloudVolume = this.worldEffectsManager.isInsideCloudVolume(cameraPos.x, cameraPos.y,
					cameraPos.z);
		...
		this.meshFaceCullingActive = !testFacesFacingAway;
	...
	public @Nullable CloudType getCurrentCloudType(double camX, double camZ) {
		return this.resolveAtmosphericOverrideCloudType(camX, camZ);
	}

	public void renderAtmosphericClouds(Matrix4f camMat, Matrix4f projMat, float partialTick, double camX,
			double camY, double camZ, float r, float g, float b) {
		if (!SimpleCloudsConfig.CLIENT.atmosphericClouds.get())
			return;

		CloudType type = this.resolveAtmosphericOverrideCloudType(camX, camZ);
		if (type != null && type != SimpleCloudsConstants.EMPTY && type.suppressesAtmosphericClouds())
			return;

		PoseStack stack = new PoseStack();
		stack.mulPose(camMat);
		this.atmosphericClouds.render(stack, projMat, partialTick, camX, camY, camZ, r, g, b);
	}

	private int getAdaptiveStormFogResolutionDivisor(float partialTick) {
		return SimpleCloudsCompatHelper.getStormFogResolutionDivisor();
	}

	private void updateAdaptiveStormFogResolution(float partialTick) {
		...
		int divisor = this.getAdaptiveStormFogResolutionDivisor(partialTick);
		if (divisor != this.stormFogResolutionDivisor) {
			this.postProcessing.resizeTargets(main, divisor);
			this.stormFogResolutionDivisor = divisor;
		}
	}
	...
	public void onResourceManagerReload(ResourceManager manager) {
		...
		this.prepareMeshGenerator(0.0F, 0.0D, 0.0D); // Prepare it
		...
		this.atmosphericClouds.init(manager);
	...
	public static void renderCloudsOpaque(CloudMeshGenerator generator, PoseStack stack, Matrix4f projMat,
		...
		renderCloudsOpaque(generator, stack, projMat, fogStart, fogEnd, partialTick, r, g, b, frustum, false,
	...
	public static void renderCloudsOpaque(CloudMeshGenerator generator, PoseStack stack, Matrix4f projMat,
			float fogStart, float fogEnd, float partialTick, float r, float g, float b, @Nullable Frustum frustum,
			boolean ditherFade) {
		renderCloudsOpaque(generator, stack, projMat, fogStart, fogEnd, partialTick, r, g, b, frustum, ditherFade,
				stack.last().pose(), IDENTITY_MATRIX, 0.0D, 0.0D, 0.0D);
	}
	...
	public static void renderCloudsOpaque(CloudMeshGenerator generator, PoseStack stack, Matrix4f projMat,
		...
		renderCloudsOpaque(generator, stack, projMat, fogStart, fogEnd, partialTick, r, g, b, frustum, false,
				viewMat, cloudWorldMat, camX, camY, camZ);
	}

	private static void renderCloudsOpaque(CloudMeshGenerator generator, PoseStack stack, Matrix4f projMat,
			float fogStart, float fogEnd, float partialTick, float r, float g, float b, @Nullable Frustum frustum,
	...
	private void renderShadowMaps(double camX, double camY, double camZ, float partialTick) {
		...
		this.renderShadowMap(this.stormFogShadowMap, this.stormFogShadowMapMatrix,
				SimpleCloudsShaders.getStormFogShadowMapShader(), null);
	...
	public float[] getCloudColor(float partialTick) {
		...
		Vec3 skyCol = this.mc.level.getSkyColor(this.mc.gameRenderer.getMainCamera().getPosition(), partialTick);
		float timeOfDay = this.mc.level.getTimeOfDay(partialTick);
		float daylight = Mth.clamp(Mth.cos(timeOfDay * ((float) Math.PI * 2.0F)) * 2.0F + 0.5F, 0.0F, 1.0F);
		float skyBlend = Mth.lerp(daylight, 0.35F, 0.15F);
		float brightness = Mth.lerp(daylight, 0.3F, 1.0F);
		float rBase = Mth.lerp(skyBlend, (float) cloudCol.x, (float) skyCol.x);
		float gBase = Mth.lerp(skyBlend, (float) cloudCol.y, (float) skyCol.y);
		float bBase = Mth.lerp(skyBlend, (float) cloudCol.z, (float) skyCol.z);

		float[] sunriseColor = this.mc.level.effects().getSunriseColor(timeOfDay, partialTick);
		if (sunriseColor != null) {
			float sunriseBlend = sunriseColor[3] * 0.35F;
			rBase = Mth.lerp(sunriseBlend, rBase, sunriseColor[0]);
			gBase = Mth.lerp(sunriseBlend, gBase, sunriseColor[1]);
			bBase = Mth.lerp(sunriseBlend, bBase, sunriseColor[2]);
		}

		float factor = this.worldEffectsManager.getDarkenFactor(partialTick, 0.8F) * brightness;
	...
	public Matrix4f createCloudWorldMatrix() {
		return new Matrix4f().translate(0.0F, (float) this.cloudManager.getCloudHeight(), 0.0F)
				.scale((float) SimpleCloudsConstants.CLOUD_SCALE);
	}
	...
	public void renderBeforeLevel(Matrix4f projMat, Matrix4f camMat, float partialTick, double camX, double camY,
		...
		if (!SimpleCloudsCompatHelper.renderThisPass())
			return;
		if (SimpleCloudsCompatHelper.isPrimaryPass())
			this.updateAdaptiveStormFogResolution(partialTick);
	...
	public boolean shouldRenderStormFog(float partialTick) {
		if (!SimpleCloudsConfig.CLIENT.renderStormFog.get())
			return false;
		return this.worldEffectsManager.getVisibleStorminessSmoothed(partialTick) > 0.01F;
	}
	...
	public void renderPreparedStormFogIntoMainTarget() {
		this.postProcessing.renderPreparedStormFogIntoMainTarget();
	}

	public void renderRawStormFogOverlay() {
		this.postProcessing.renderRawStormFogOverlay();
	}

	public void doScreenSpaceWorldFog(Matrix4f camMat, Matrix4f projMat, float partialTick) {
		this.doScreenSpaceWorldFog(camMat, projMat, partialTick, this.fogStart, this.fogEnd);
	}
	...
	public void doScreenSpaceWorldFog(Matrix4f camMat, Matrix4f projMat, float partialTick, float fogStart,
		...
		this.postProcessing.doScreenSpaceWorldFog(partialTick, effect -> {
			...
			// Use renderer fog values rather than RenderSystem state so the storm fog
			// compositing is not broken when DH overrides the global fog range.
			effect.safeGetUniform("FogStart").set(fogStart);
			effect.safeGetUniform("FogEnd").set(fogEnd);
	...
	public void doFinalCompositePass(Matrix4f camMat, float partialTick, Matrix4f projMat,
		...
		this.postProcessing.doFinalCompositePass(partialTick, effect -> {
			...
			WeightedBlendingTarget transparencyTarget = this.postProcessing.getCloudTransparencyTarget();
			if (this.settings.useTransparency() && transparencyTarget != null) {
				effect.setSampler("AccumTexture", transparencyTarget::getColorTextureId);
				effect.setSampler("RevealageTexture", transparencyTarget::getRevealageTextureId);
			}
	...
	public void doStormPostProcessing(Matrix4f camMat, float partialTick, Matrix4f projMat, double camX, double camY,
		...
		this.doStormPostProcessing(camMat, partialTick, projMat, camX, camY, camZ, r, g, b,
				this.mc.getMainRenderTarget()::getDepthTextureId);
	}

	public void doStormPostProcessing(Matrix4f camMat, float partialTick, Matrix4f projMat, double camX, double camY,
			double camZ, float r, float g, float b, java.util.function.IntSupplier depthSampler) {
		if (this.stormFogShadowMap == null || this.stormFogShadowMapMatrix == null)
			return;
		...
		this.postProcessing.doStormPostProcessing(partialTick, effect -> {
			effect.setSampler("DepthSampler", depthSampler);
			effect.safeGetUniform("InverseWorldProjMat").set(this.inverseProjMatrix);
			effect.safeGetUniform("InverseModelViewMat").set(this.inverseModelViewMatrix);
			...
			float factor = this.worldEffectsManager.getVisibleDarkenFactor(partialTick,
					WorldEffects.EFFECTS_STRENGTH_MULTIPLER);
			effect.safeGetUniform("CutoffDistance").set(1000.0F * factor);
			effect.safeGetUniform("LightTransmittenceDistance").set(500.0F);
			effect.safeGetUniform("DistantDensityMultiplier").set(1.0F);
			effect.safeGetUniform("NearFadeStart").set(0.0F);
			effect.safeGetUniform("NearFadeEnd").set(0.0F);
	...
	public static void prepareShader(ShaderInstance shader, Matrix4f modelView, Matrix4f projMat, float fogStart,
			float fogEnd) {
		prepareShader(shader, modelView, projMat, fogStart, fogEnd, modelView, IDENTITY_MATRIX, 0.0D, 0.0D, 0.0D);
	}

	public static void prepareShader(ShaderInstance shader, Matrix4f modelView, Matrix4f projMat, float fogStart,
			float fogEnd, Matrix4f viewMat, Matrix4f cloudWorldMat, double camX, double camY, double camZ) {
		for (int i = 0; i < 12; ++i) {
		...
		shader.safeGetUniform("ViewMat").set(viewMat);
