...
public class AtmosphericCloudsRenderHandler {
    ...
    private static final Formation DEFAULT = new Formation(b -> b.value().getBaseTemperature() < 2.0F, 0.6F, 1.0F,
            1.0F);
    private static final ImmutableList<Formation> FORMATIONS = Util.make(() -> {
        ImmutableList.Builder<Formation> builder = ImmutableList.builder();
        builder.add(new Formation(
                b -> b.is(Tags.Biomes.IS_COLD_OVERWORLD) || b.is(Tags.Biomes.IS_DRY_OVERWORLD)
                        || b.is(BiomeTags.IS_SAVANNA),
                0.3F, 1.0F, 30.0F));
        builder.add(new Formation(b -> b.is(Tags.Biomes.IS_HOT_OVERWORLD), 0.8F, 10.0F, 10.0F));
        builder.add(new Formation(b -> b.is(Tags.Biomes.IS_PLAINS) || b.is(BiomeTags.IS_FOREST), 1.0F, 2.0F, 10.0F));
        builder.add(DEFAULT);
    ...
    private @Nullable Formation nextFormation;
    ...
    public void render(PoseStack stack, Matrix4f projMat, float partialTick, double camX, double camY, double camZ,
        ...
        updatePass(passes.get(0), invertedProjMat, invertedModelViewMat, camX, camY, camZ, yaw, shiftMovement,
                1.0F - transition, this.formation, r, g, b);
        updatePass(passes.get(1), invertedProjMat, invertedModelViewMat, camX, camY, camZ, yaw, shiftMovement,
    ...
    private static void updatePass(PostPass pass, Matrix4f invertedProjMat, Matrix4f invertedModelViewMat,
            double camX, double camY, double camZ, float yaw, float shiftMovement, float densityMult,
            Formation formation, float r, float g, float b) {
    ...
    public void init(ResourceManager manager) {
        ...
        try {
            this.postProcessingShader = new PostChain(this.mc.getTextureManager(), manager,
